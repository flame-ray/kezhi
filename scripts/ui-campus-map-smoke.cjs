 ...
 try {
  ...
  await context.addInitScript(()=>{
   ...
   window.__TAURI_INTERNALS__={transformCallback:()=>++id,unregisterCallback:()=>{},invoke:async(command,args)=>{
    ...
    if(command==='sync_course_widget')return;
  ...
  await map.getByLabel('查看楼栋',{exact:true}).selectOption('south-lab');await settle();
  assert.match(await map.locator('.campus-location-summary').textContent(),/医学院/);
  assert.equal(await map.getByRole('option',{name:'实验楼',exact:true}).count(),0);
  assert.match(await map.locator('image').getAttribute('href'),/campus-overview-v2/);
  await page.screenshot({path:path.join(out,'medical-building.png')});
  await map.getByLabel('查看楼栋',{exact:true}).selectOption('qiushi-c');await settle();
