...
export function createImportSafetyBackup(snapshot: ScheduleSnapshot, reason: string, now = new Date()): ImportSafetyBackup {
  ...
  const data = JSON.stringify({ ...snapshot, format: "kezhi-schedule", version: 1, importBackup: undefined, grades: undefined, exams: undefined, selectionAssistant: undefined });
...
export function restoreImportSafetyBackup(current: ScheduleSnapshot): ScheduleSnapshot {
  ...
  return { ...restored, grades: current.grades, exams: current.exams, selectionAssistant: current.selectionAssistant, importBackup: createImportSafetyBackup(current, "恢复自动备份前") };
}
